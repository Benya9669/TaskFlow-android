...
class TaskListViewModel(private val tasks: TaskRepository, private val sync: SyncRepository) : ViewModel() {
    ...
    fun createInboxTask(title: String) {
        ...
        viewModelScope.launch {
            runCatching { tasks.createInboxTask(title) }
            refresh()
        }
    }

    fun completeTask(taskId: String) {
        viewModelScope.launch { runCatching { tasks.completeLocalTask(taskId) }; refresh() }
    }
...
class TaskListViewModelFactory(private val database: ru.taskflow.app.data.local.TaskFlowDatabase, private val tokenStore: TokenStore) : ViewModelProvider.Factory {
    @Suppress("UNCHECKED_CAST")
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        val tasks = TaskRepository(database)
        val serverUrl = checkNotNull(tokenStore.serverUrl()) { "Сервер не настроен" }
        return TaskListViewModel(tasks, SyncRepository(TaskFlowApiFactory(tokenStore).create(serverUrl), tasks)) as T
    }
}
