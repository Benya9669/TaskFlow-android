...
fun TaskFlowApp(sharedText: String? = null, taskIdFromLink: String? = null, verificationServerUrl: String? = null, verificationToken: String? = null) {
    ...
    val kanbanBusy by taskListViewModel.kanbanBusy.collectAsStateWithLifecycle()
    ...
    if (wideLayout) {
        androidx.compose.foundation.layout.Row(Modifier.fillMaxSize()) {
            ...
            TaskFlowContent(destination, PaddingValues(), tasks, projects, archivedProjects, kanbanColumns, selectedProjectId, taskIdFromLink, taskListViewModel::createInboxTask, taskListViewModel::completeTask, taskListViewModel::reopenTask, taskListViewModel::moveTask, taskListViewModel::deleteTask, updateTask, taskListViewModel::refresh, syncing, pendingCount, syncError, taskListViewModel::dismissError, projectBusy, kanbanBusy, openProjectTasks, taskListViewModel::createProject, taskListViewModel::updateProject, taskListViewModel::archiveProject, taskListViewModel::restoreProject, taskListViewModel::createKanbanTask, taskListViewModel::createKanbanColumn, taskListViewModel::updateKanbanColumn, taskListViewModel::reorderKanbanColumns, taskListViewModel::deleteKanbanColumn, TokenStore(context).serverUrl(), notificationsEnabled, profile, notificationSettings, sessionViewModel::updateProfile, sessionViewModel::logout) { selectedProjectId = null; destination = Destination.Projects }
        }
    } else {
        Scaffold(bottomBar = {
            NavigationBar {
...
@Composable
private fun TaskFlowContent(destination: Destination, padding: PaddingValues, tasks: List<ru.taskflow.app.data.local.TaskEntity>, projects: List<ru.taskflow.app.data.local.ProjectEntity>, archivedProjects: List<ru.taskflow.app.data.local.ProjectEntity>, kanbanColumns: List<ru.taskflow.app.data.local.KanbanColumnEntity>, selectedProjectId: String?, taskIdFromLink: String?, onCreateInboxTask: (String, String, String?) -> Unit, onCompleteTask: (String) -> Unit, onReopenTask: (String) -> Unit, onMoveTask: (String, ru.taskflow.app.data.local.KanbanColumnEntity, String?) -> Unit, onDeleteTask: (String) -> Unit, onUpdateTask: (String, ru.taskflow.app.data.TaskUpdate) -> Unit, onRefresh: () -> Unit, syncing: Boolean, pendingCount: Int, syncError: String?, onDismissError: () -> Unit, projectBusy: Boolean, kanbanBusy: Boolean, onOpenProjectTasks: (String) -> Unit, onCreateProject: (String, String) -> Unit, onUpdateProject: (ru.taskflow.app.data.local.ProjectEntity, String, String) -> Unit, onArchiveProject: (ru.taskflow.app.data.local.ProjectEntity) -> Unit, onRestoreProject: (ru.taskflow.app.data.local.ProjectEntity) -> Unit, onCreateKanbanTask: (ru.taskflow.app.data.local.KanbanColumnEntity, String, String?) -> Unit, onCreateKanbanColumn: (String, String, String) -> Unit, onUpdateKanbanColumn: (ru.taskflow.app.data.local.KanbanColumnEntity, String, String, String) -> Unit, onReorderKanbanColumns: (List<ru.taskflow.app.data.local.KanbanColumnEntity>) -> Unit, onDeleteKanbanColumn: (ru.taskflow.app.data.local.KanbanColumnEntity, ru.taskflow.app.data.local.KanbanColumnEntity) -> Unit, serverUrl: String?, notificationsEnabled: Boolean, profile: ru.taskflow.app.ui.auth.ProfileUiState, onOpenNotificationSettings: () -> Unit, onSaveProfile: (String, String) -> Unit, onLogout: () -> Unit, onBackToProjects: () -> Unit) {
    ...
    Scaffold(topBar = { TopAppBar(title = { Text(selectedProject?.name ?: destination.label, style = MaterialTheme.typography.titleLarge) }, navigationIcon = { if (selectedProject != null) androidx.compose.material3.IconButton(onClick = onBackToProjects) { Icon(Icons.AutoMirrored.Outlined.ArrowBack, "Вернуться к проектам") } }, actions = { if (pendingCount > 0) Text("$pendingCount ожидает", style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.primary, modifier = Modifier.padding(end = TaskFlowSpace.md)) }) }) { contentPadding ->
        Column(
            ...
            when (destination) {
                ...
                Destination.Board -> KanbanBoardContent(kanbanColumns, tasks, projects, kanbanBusy, syncing, syncError, onDismissError, onRefresh, onMoveTask, onCreateKanbanTask, onCreateKanbanColumn, onUpdateKanbanColumn, onReorderKanbanColumns, onDeleteKanbanColumn)
