...
The `0.3.0-beta.3` foundation includes a Kotlin + Jetpack Compose shell, a web-aligned TaskFlow design system and adaptive launcher icon, encrypted session storage, Room cache, bounded pull sync, durable task/project outboxes with explicit conflict resolution, task search/sort/detail editing, project management, a drag-and-drop Kanban board with manual ordering, and profile settings.
...
- `versionName` uses SemVer: `MAJOR.MINOR.PATCH` with optional prerelease suffix, for example `0.3.0-beta.3`.
