...
The `0.3.0-beta.2` foundation includes a Kotlin + Jetpack Compose shell, a web-aligned TaskFlow design system and adaptive launcher icon, encrypted session storage, Room cache, bounded pull sync, durable task/project outboxes with explicit conflict resolution, task search/sort/detail editing, project management, a Kanban board, and profile settings. Kanban moves are offline-first; drag-and-drop and manual card ordering remain planned.
...
- `versionName` uses SemVer: `MAJOR.MINOR.PATCH` with optional prerelease suffix, for example `0.3.0-beta.2`.
